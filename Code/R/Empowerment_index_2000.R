...
path_in <- paste0(
  base_directory,
  "/Data/Data_raw/Ethiopia/ET_2000_DHS_Standard/ETIR41FL.DTA"
)
...
# ---- Age band parameters ----
age_breaks  <- c(15, 25, 35, 50)  # 50 to include 49
age_labels  <- c("15-24", "25-34", "35-49")
...
recode_decision <- function(x) {
  ...
  dplyr::case_when(
    ...
    x %in% c(4, 5)    ~ 1L,     # husband alone / someone else / other -> deprived
    TRUE                 ~ NA_integer_
  )
}
...
agency_raw <- ir %>%
  transmute(
    ...
    # --- Domain B: Attitudes to wife-beating ---
    B_v744a = recode_beating(v744a),
    B_v744b = recode_beating(v744b),
    B_v744c = recode_beating(v744c),
    B_v744d = recode_beating(v744d),
    B_v744e = recode_beating(v744e),
    
    # --- Domain A: Decision autonomy ---
    #A_v739  = recode_decision(v739)
  )

## THE BELOW HASHED OUT CODE IS THE ORIGINAL WE ARE TRYING THE NEW CODE 
...
summ_long_unw <- agency_scored_2000 %>%
...
### also look into unweighted 

# Unweighted design (every observation weight = 1)
des_00_unw <- svydesign(ids = ~1, weights = ~1, data = agency_scored_2000)

region_means_00_unw <- svyby(
  ~c_score,
  ~region,
  design = des_00_unw,
  FUN = svymean,
